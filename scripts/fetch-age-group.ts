...
async function processPlayer(israeliId: number, name: string, israeliRating: number, workerId: number): Promise<void> {
  ...
  try {
    ...
    for (const game of games) {
      ...
      const isNew = await storeGame(tournamentId, mainPlayerId, game, opponentId, board, dbEntry.entry.startingRating || undefined);
      if (isNew) saved++;
      else backfilled++;
    }

    console.log(`[W${workerId}] ✓ ${name} (${israeliId}, ${israeliRating}): ${tournaments.length} tournaments, ${saved} new + ${backfilled} elo-backfilled / ${games.length} games, ${withRatings.length} rating history entries`);
  } catch (err) {
...
async function main() {
  ...
  console.log(`=== Fetching game history: age < 18 (born > ${currentYear - 18}), ELO > 1700 ===\n`);
  ...
  const target = allPlayers
    .filter(p => p.birthYear && (currentYear - p.birthYear) < 18 && p.israeliRating > 1700)
    .sort((a, b) => b.israeliRating - a.israeliRating);

  console.log(`Found ${target.length} players matching criteria\n`);
  for (const p of target) {
    console.log(`  ${p.name} (${p.birthYear}, ${p.israeliRating})`);
  }
  console.log();
